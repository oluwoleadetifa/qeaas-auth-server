...
## Usage
### Entropy Modes
The auth server supports two runtime entropy configurations:
...
```cargo run --release -- loadtest \
  ...
  --md-out results.md
```
3. Compare Config 1 and Config 2
Run the auth server once with `ENTROPY_MODE=direct_qrng` and once with
`ENTROPY_MODE=hybrid_csprng`, reusing the same enrolled `users.jsonl` if the
server enrollment state is preserved for that run. Append both benchmark
results to the same CSV/Markdown files by changing only `--label`.
...
## Current Limitations
Hybrid CSPRNG mode is implemented, but still needs full Ubuntu 18 benchmark validation
