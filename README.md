...
## Usage
### Entropy Modes
The auth server supports three runtime entropy configurations:

- `ENTROPY_MODE=direct_qrng` - Config 1, direct QRNG fetch per entropy request.
- `ENTROPY_MODE=hybrid_csprng` - Config 2, QRNG-seeded ChaCha20 CSPRNG with byte-count reseeding.
- `ENTROPY_MODE=parallel_hybrid` - Config 3, QRNG-seeded sharded ChaCha20 CSPRNG pool. `hybrid_pool` is accepted as an alias.

Hybrid mode settings:

- `HYBRID_QRNG_SEED_SIZE` - QRNG seed bytes fetched for the CSPRNG, default `32`.
- `HYBRID_RESEED_AFTER_BYTES` - reseed after this many generated bytes, default `1048576`.
- `HYBRID_POOL_SIZE` - number of independent CSPRNG shards for Config 3, default `8`.
...
```cargo run --release -- loadtest \
  ...
  --md-out results.md
```
3. Run Config 3 Load Test
Start the auth server with:
```ENTROPY_MODE=parallel_hybrid HYBRID_POOL_SIZE=8 cargo run --release```
...
```cargo run --release -- loadtest \
  ...
  --concurrency 10 \
  --duration 30 \
  --label parallel_hybrid \
  --csv-out results.csv \
  --md-out results.md
```

4. Compare Configs
Run the auth server once per mode with `ENTROPY_MODE=direct_qrng`,
`ENTROPY_MODE=hybrid_csprng`, and `ENTROPY_MODE=parallel_hybrid`, reusing the
same enrolled `users.jsonl` if the server enrollment state is preserved for
that run. Append results to the same CSV/Markdown files by changing only
`--label`. Config 3 reduces CSPRNG mutex contention by routing requests across
independently seeded ChaCha20 RNG shards instead of one shared RNG mutex.
...
## Current Limitations
Hybrid CSPRNG and parallel hybrid modes are implemented, but still need full Ubuntu 18 benchmark validation
