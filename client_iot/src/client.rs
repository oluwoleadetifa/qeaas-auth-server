...
impl IotClient {
    pub fn new(auth_base: String) -> Self {
        Self { http: Client::new(), auth_base }
    }
    ...
    pub async fn enroll(&self, device_id: &str, pq: &DevicePq) -> anyhow::Result<EnrollResponse> {
        ...
        println!("ENTROPY_REQUEST_JSON={}", serde_json::to_string(&req).unwrap());
...
let ct = general_purpose::STANDARD.decode(&resp.kem_ct_b64).context("bad kem_ct_b64")?;
let aead_nonce_vec = general_purpose::STANDARD.decode(&resp.aead_nonce_b64).context("bad aead_nonce_b64")?;
let entropy_ct = general_purpose::STANDARD.decode(&resp.entropy_ct_b64).context("bad entropy_ct_b64")?;
let server_sig = general_purpose::STANDARD.decode(&resp.server_signature_b64).context("bad server_signature_b64")?;
...
let server_sig_pk_bytes =
    general_purpose::STANDARD.decode(server_sig_pk_b64).context("bad server_sig_pk_b64")?;
let tbs = crate::pq::build_server_tbs(device_id, n, &nonce, &ct, &aead_nonce12, &entropy_ct);
